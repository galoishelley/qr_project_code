var viewModel =
{
	...
	cNewContact : ko.observable(),
	cUpdateContact : ko.observable(),
	cRemoveContact : ko.observable(),
	...
	// Student Model
	studentModel : function(sID, sFname, sMname, sLname, sAddress, sNotes,
			sType)
	{
		var self = this;
...
{
	alert('ssave');
	var self = this, callback = 'successMsg';
...
viewModel.cAddSave = function()
{
	var self = this, callback = 'successMsg';

	if (viewModel.edit() == false)
	{
		this.cNewContact = new viewModel.contactModel(

		viewModel.cID(), viewModel.cFName(), viewModel.cLName(), viewModel
				.cEmail(), viewModel.cTitle(), viewModel.cPhone(), viewModel
				.cNotes());
		viewModel.cList.push(this.cNewContact);
		self.onRequest('create', successMsg, callback);
	}
...
this.cDelete = function(index)
{
	...
	if (confirm('Are you sure you want to delete this item?'))
	{
		...
		viewModel.cRemoveContact = index;

		viewModel.onRequest('remove', successMsg, callback);
	}
...
function successMsg()
{
	$.getJSON('classes/class.main.php', function(data)
	{
